...
pub fn build_pprf(
        ...
        .enumerate()
    {
        ...
        s_tilda_hash.challenge_bytes(&ALL_BUT_ONE_PPRF_HASH_LABEL, &mut out.s_tilda);
    }
}
...
pub fn eval_pprf(
    ...
    for (j, out) in output.iter().enumerate() {
        let x_star_0: u8 = receiver_ot_seed.choice_bits.extract_bit(j * SOFT_SPOKEN_K) as u8;

        let mut s_star_i = [[0u8; LAMBDA_C_BYTES]; SOFT_SPOKEN_Q];

        s_star_i[x_star_0 as usize] = receiver_ot_seed.otp_dec_keys[j * SOFT_SPOKEN_K];
        ...
        for i in 1..SOFT_SPOKEN_K {
            ...
            for y in 0..(1 << i) {
                ...
                (0..LAMBDA_C_BYTES).for_each(|b_i| {
                    s_star_i_plus_1[2 * y][b_i].conditional_assign(&temp_0[b_i], choice);
                    s_star_i_plus_1[2 * y + 1][b_i].conditional_assign(&temp_1[b_i], choice);
                });
            }

            let x_star_i: u8 = 1 ^ receiver_ot_seed
                .choice_bits
                .extract_bit(j * SOFT_SPOKEN_K + i) as u8;

            let big_f_i_star = &receiver_ot_seed.otp_dec_keys[j * SOFT_SPOKEN_K + i];
            ...
            for b_i in 0..LAMBDA_C_BYTES {
                ...
                for y in 0..2usize.pow(i as u32) {
                    ...
                    s_star_i_plus_1[2 * y_star + ct_x][b_i].conditional_assign(&temp_byte, choice);
                }
            }
        ...
        for y in 0..SOFT_SPOKEN_Q {
            ...
            (0..LAMBDA_C_BYTES * 2).for_each(|b_i| {
                let temp_byte = s_tilda_star_y_star[b_i] ^ s_tilda_star[y][b_i];
                s_tilda_star_y_star[b_i].conditional_assign(&temp_byte, choice);
            })
        }
        ...
        s_tilda_hash.challenge_bytes(&ALL_BUT_ONE_PPRF_HASH_LABEL, &mut s_tilda_digest);
...
mod test {
    ...
    use crate::sl_oblivious::endemic_ot::OneTimePadEncryptionKeys;
    ...
    fn generate_seed_ot_for_test() -> (SenderOutput, ReceiverOutput) {
        ...
        let receiver_ot_seed = ReceiverOutput::new(random_choices, one_time_pad_enc_keys);
