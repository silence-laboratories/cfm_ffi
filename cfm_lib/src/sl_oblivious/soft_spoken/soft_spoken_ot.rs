...
use crate::sl_oblivious::{
    constants::{
        SOFT_SPOKEN_EXPAND_LABEL, SOFT_SPOKEN_LABEL, SOFT_SPOKEN_MATRIX_HASH_LABEL,
...
pub struct SenderOTSeed {
    pub otp_enc_keys: [[[u8; LAMBDA_C_BYTES]; SOFT_SPOKEN_Q]; LAMBDA_C / SOFT_SPOKEN_K],
}

impl Default for SenderOTSeed {
    fn default() -> Self {
        Self {
            otp_enc_keys: [[[0u8; LAMBDA_C_BYTES]; SOFT_SPOKEN_Q]; LAMBDA_C / SOFT_SPOKEN_K],
        }
    }
}
...
pub struct ReceiverOTSeed {
    ...
    pub otp_dec_keys: [[[u8; LAMBDA_C_BYTES]; SOFT_SPOKEN_Q]; LAMBDA_C / SOFT_SPOKEN_K],
}
...
impl Default for ReceiverOTSeed {
    fn default() -> Self {
        Self {
            ...
            otp_dec_keys: [[[0u8; LAMBDA_C_BYTES]; SOFT_SPOKEN_Q]; LAMBDA_C / SOFT_SPOKEN_K],
        }
    }
}
...
pub struct SenderExtendedOutput {
    pub v_0: Vec<[[u8; KAPPA_BYTES]; L]>, // [[[u8; KAPPA_BYTES]; OT_WIDTH]; L]
    pub v_1: Vec<[[u8; KAPPA_BYTES]; L]>, // [[[u8; KAPPA_BYTES]; OT_WIDTH]; L]
}

impl SenderExtendedOutput {
    pub fn new(l_batch: usize) -> Box<Self> {
        Box::new(SenderExtendedOutput {
            v_0: vec![[[0u8; KAPPA_BYTES]; L]; l_batch + RHO],
            v_1: vec![[[0u8; KAPPA_BYTES]; L]; l_batch + RHO],
        })
    }
}

/// ReceiverExtendedOutputVX
#[derive(Clone, Serialize, Deserialize)]
pub struct ReceiverExtendedOutputVX {
    #[serde(with = "serde_arrays")]
    pub inner: [[u8; KAPPA_BYTES]; L], // [[[u8; KAPPA_BYTES]; OT_WIDTH]; L]
}

impl Default for ReceiverExtendedOutputVX {
    fn default() -> Self {
        Self {
            inner: [[0u8; KAPPA_BYTES]; L],
        }
    }
}
...
pub struct ReceiverExtendedOutput {
    ...
    pub v_x: Vec<ReceiverExtendedOutputVX>, // [[[u8; KAPPA_BYTES]; OT_WIDTH]; L]
}

impl ReceiverExtendedOutput {
    pub(crate) fn new(choices: &[u8; L_BYTES], l_batch: usize) -> Box<Self> {
        Box::new(ReceiverExtendedOutput {
            choices: *choices,
            v_x: vec![ReceiverExtendedOutputVX::default(); l_batch + RHO],
        })
    }
}
...
impl SoftSpokenOTReceiver {
    pub fn process<R: RngCore + CryptoRng>(
        ...
        let mut r_x = [[[0u8; L_PRIME_BYTES]; LAMBDA_C_DIV_SOFT_SPOKEN_K]; SOFT_SPOKEN_Q];
        ...
        matrix_hasher.challenge_bytes(&SOFT_SPOKEN_MATRIX_HASH_LABEL, &mut digest_matrix_u);
        ...
        for j in 0..SOFT_SPOKEN_M {
            ...
            for (x_k, x_hat_k) in output.x.iter_mut().zip(&x_hat_j_times_chi_j) {
                *x_k ^= x_hat_k;
            }
        ...
        for (ot_width_index, v_x) in extended_output.v_x.iter_mut().enumerate() {
            for (j, v_x_j) in v_x.inner.iter_mut().enumerate() {
                let mut t = Transcript::new(&SOFT_SPOKEN_LABEL);
                t.append_message(b"sid", session_id);
                t.append_u64(b"i", ot_width_index as u64);
                t.append_u64(b"j", j as u64);
                t.append_message(&SOFT_SPOKEN_RANDOMIZE_LABEL, &psi[j]);
                t.challenge_bytes(b"", v_x_j);
            }
        }
    }
}
...
impl SoftSpokenOTSender {
    pub fn process(
        ...
        let mut r_x = bytemuck::allocation::zeroed_box::<
            [[[u8; L_PRIME_BYTES]; LAMBDA_C_DIV_SOFT_SPOKEN_K]; SOFT_SPOKEN_Q],
        >();
        ...
        for i in 0..LAMBDA_C_DIV_SOFT_SPOKEN_K {
            for (j, rx_j) in r_x.iter_mut().enumerate() {
                if j == seed_ot_results.random_choices[i] as usize {
                    ...
                    t.append_message(b"", &seed_ot_results.otp_dec_keys[i][j]);
                    t.challenge_bytes(&SOFT_SPOKEN_EXPAND_LABEL, &mut rx_j[i]);
                }
            }
        }
        ...
        for i in 0..LAMBDA_C_DIV_SOFT_SPOKEN_K {
            ...
            for bit_index in 0..SOFT_SPOKEN_K {
                ...
                for k in 0..L_PRIME_BYTES {
                    w_matrix[i * SOFT_SPOKEN_K + bit_index][k] ^= delta_i_mask & message.u[i][k];
                }
            }
        ...
        let mut digest_matrix_u = [0u8; 32];
        hash_matrix_u.challenge_bytes(&SOFT_SPOKEN_MATRIX_HASH_LABEL, &mut digest_matrix_u);

        let chi_matrix: [[u8; S_BYTES]; SOFT_SPOKEN_M] = array::from_fn(|j| {
            let mut ts = Transcript::new(b"");
            ts.append_u64(b"index", j as u64);
            ts.append_message(b"", &digest_matrix_u);
        ...
        for (i, w_matrix_i) in w_matrix.iter().enumerate() {
            ...
            let t_i_plus_delta_i_times_x: [u8; S_BYTES] =
                array::from_fn(|k| message.t[i][k] ^ (bit_mask & message.x[k]));
        ...
        let mut output = SenderExtendedOutput::new(l_batch);

        let v_0 = &mut output.v_0;
        let v_1 = &mut output.v_1;

        #[allow(clippy::needless_range_loop)]
        for ot_width_index in 0..(l_batch + RHO) {
            #[allow(clippy::needless_range_loop)]
            for j in 0..L {
                let mut t = Transcript::new(&SOFT_SPOKEN_LABEL);
                t.append_message(b"sid", session_id);
                ...
                t.challenge_bytes(b"", &mut v_0[ot_width_index][j]);
            }
        }
        #[allow(clippy::needless_range_loop)]
        for j in 0..L {
        ...
        for ot_width_index in 0..(l_batch + RHO) {
            ...
            for j in 0..L {
                ...
                t.append_u64(b"j", j as u64);
                t.append_message(&SOFT_SPOKEN_RANDOMIZE_LABEL, &zeta[j]);
                t.challenge_bytes(b"", &mut v_1[ot_width_index][j]);
            }
        }
...
pub fn generate_all_but_one_seed_ot<R: RngCore + CryptoRng>(
    rng: &mut R,
    ...
    for i in 0..(LAMBDA_C_DIV_SOFT_SPOKEN_K) {
        ...
        receiver_ot_seed.otp_dec_keys[i][choice as usize] = [0u8; LAMBDA_C_BYTES];
    }
...
mod tests {
    ...
    fn soft_spoken() {
        ...
        let mut receiver_extended_output = ReceiverExtendedOutput::new(&choices, l_batch);
        ...
        let sender_extended_output =
            SoftSpokenOTSender::process(&session_id, &receiver_ot_results, &round1, l_batch)
                .unwrap();
        // println!("Round2: {:?}", start.elapsed());

        for i in 0..L {
            let bit = choices.extract_bit(i);
            let receiver_choice_bit = receiver_extended_output.choices.extract_bit(i);
            assert_eq!(bit, receiver_choice_bit);

            for k in 0..(l_batch + RHO) {
                let receiver_value = receiver_extended_output.v_x[k].inner[i];
                let sender_value = match bit {
                    true => sender_extended_output.v_1[k][i],
                    false => sender_extended_output.v_0[k][i],
                };
